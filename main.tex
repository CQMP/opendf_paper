...
We provide the practical illustration of the method for the Hubbard model in $D=2$ dimensions. We show the $k$-dependence of the real part of the lattice self-energy at $i\omega = i\pi/\beta$ in Fig.~\ref{fig:hub_atomic} for the case of particle-hole symmetry at $U/t=8,12$ and compare it with available data from the Dynamical Cluster Approximation \cite{MaierJarrell:2005}. Panel ... shows the comparison of the frequency dependence of the imaginary parts at $k=(0,\pi)$. The data from the ALPS DMFT \cite{Albuquerque2007}\cite{ALPSDMFT} package with CT-AUX solver was used as an input. The DMFT self-energy is $k$-independent and is $\Re \Sigma_{\omega k} = 0$. Taking into account the spatially dependent corrections by the dual fermions leads to a correct k-dependence of the self-energy. The comparison between multiple methods will be discussed elsewhere []. 
