...
\begin{abstract}
We present \textbf{opendf} code, an open-source implementation of the dual fermion method : a multiscale approach to solve lattice problems of interacting strongly correlated systems. Our code provides is applicable to Hubbard models in dimensions $D=1,2,3,4$. The method is built on the dynamical mean field theory as a starting point and hence utilizes the DMFT input: Green's function and full reducible vertex functions. As an output, the code provides an estimate for spatially dependent static and dynamic susceptibilities and Green's functions. The code is distributed as an open-source package under MIT license. 
\end{abstract}
...
As a first step, performed outside of this code, an approximate solution of the model is obtained from a dynamical mean field calculation. 
...
The presented version of the code considers spin-symmetric solutions of the lattice $s=1/2$ fermionic problems, and does not describe symmetry-broken phases. We will omit the spin index $\sigma$ from now on. The complete set of input quantities reads
\begin{itemize}
...
\caption{(a) Execution time; (b) Error $\delta_g$ of the dual fermion Green's function $\tilde G_{i\omega, k}$ at $i\omega = i\pi / \beta, k = (0,0)$ for the Hubbard model in 2 dimensions at different number of bosonic frequencies $N_b$. (c) $\delta_g(N_b)$ in a logarithmic scale. }
\label{fig:benchmark_b}
\end{figure}

Fig. \ref{fig:benchmark_b} shows the performance of the \texttt{opendf} code upon the change of the total number of bosonic frequencies in the vertex $\gamma_{\Omega}$. The computational effort grows linearly in $N_{\Omega}$, while the error is f the order of few percent and decreases quadratically, indicating that this error can be eliminated with large enough $N_{\Omega}$.
...
We analyze the performance of the code with respect to the change of the total number of fermionic frequencies on Fig. \ref{fig:benchmark_f}. The computational scaling grows quadratically, however the relative error an order of magnitude smaller, as compared to the case of different $N_{\Omega}$. The  convergence of $g$ is achived very quickly upon an increase of $N_{\omega}$ and there is no scaling at large $N_{\omega}$.
...
The performance of the code with respect to the change of number of points, sampling the Brilloin zone, is plotted on Fig. \ref{fig:benchmark_f}. The computational effort scales linearly with the volume $N_k^d$ of the system and the error is very small and scales as $(N_k^d)^6$, until it reaches the floating point precision.
...
Authors are grateful to D. Hirschmeier for fruitful discussions and acknowedge the Simons collaboration on many-electron problem for financial support and for it's support of the \texttt{ALPSCore} project.
